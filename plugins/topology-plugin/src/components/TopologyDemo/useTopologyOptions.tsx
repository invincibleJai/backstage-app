...
import { Controller, GraphModel, Model, NodeShape } from '@patternfly/react-topology';
...
export const useTopologyOptions = (
  ...
  const layoutDropdown = (
    <Split>
      <SplitItem style={{display: 'flex', alignItems: 'center'}}>
        <span className="pf-u-display-inline-block pf-u-mr-md pf-u-mt-sm">Layout</span>
      </SplitItem>
      <SplitItem>
        <Dropdown
  ...
  const renderNodeOptionsDropdown = () => {
    const selectContent = (
      <div>
        ...
        <SelectOption
          value="Status"
          isChecked={!!(nodeOptions?.statuses?.length && nodeOptions.statuses.length > 1)}
          onClick={() =>
            setNodeOptions(prev => ({
              ...prev,
              statuses: prev?.statuses?.length && prev.statuses.length > 1 ? DefaultNodeOptions.statuses : NODE_STATUSES
            }))
          }
        />
        <SelectOption
  ...
  const toggleNodeShape = (shape: NodeShape): void => {
    ...
    if (index && index >= 0) {
      setNodeOptions(prev => ({
        ...prev,
        shapes: [...(prev?.shapes as NodeShape[])?.slice(0, index), ...(prev?.shapes as NodeShape[])?.slice(index + 1)]
      }));
    } else {
      setNodeOptions(prev => ({
        ...prev,
        shapes: [...(prev?.shapes as NodeShape[]), shape]
      }));
    }
  };
  ...
  const renderEdgeOptionsDropdown = () => {
    const selectContent = (
      <div>
        <SelectOption
          value="Status"
          isChecked={!!(edgeOptions?.edgeStatuses?.length && edgeOptions?.edgeStatuses?.length > 1)}
          onClick={() =>
            setEdgeOptions(prev => ({
              ...prev,
              edgeStatuses: prev?.edgeStatuses?.length && prev?.edgeStatuses?.length > 1 ? DefaultEdgeOptions.edgeStatuses : NODE_STATUSES
            }))
          }
        />
        <SelectOption
          value="Styles"
          isChecked={!!(edgeOptions?.edgeStyles?.length && edgeOptions.edgeStyles.length > 1)}
          onClick={() =>
            setEdgeOptions(prev => ({
              ...prev,
              edgeStyles: prev?.edgeStyles?.length && prev.edgeStyles.length > 1 ? DefaultEdgeOptions.edgeStyles : EDGE_STYLES
            }))
          }
        />
        <SelectOption
          value="Animations"
          isChecked={!!(edgeOptions?.edgeAnimations?.length && edgeOptions.edgeAnimations.length > 1)}
          onClick={() =>
            setEdgeOptions(prev => ({
              ...prev,
              edgeAnimations: prev?.edgeAnimations?.length && prev.edgeAnimations.length > 1 ? DefaultEdgeOptions.edgeAnimations : EDGE_ANIMATION_SPEEDS
            }))
          }
        />
        <SelectOption
          value="Terminal types"
          isChecked={!!(edgeOptions?.terminalTypes?.length && edgeOptions.terminalTypes.length > 1)}
          onClick={() =>
            setEdgeOptions(prev => ({
              ...prev,
              terminalTypes: prev?.terminalTypes?.length && prev.terminalTypes.length > 1 ? DefaultEdgeOptions.terminalTypes : EDGE_TERMINAL_TYPES
            }))
          }
        />
        <SelectOption
  ...
  const restoreLayout = () => {
    if (savedModel) {
      ...
      currentModel.graph = {
        ...currentModel.graph as GraphModel,
      ...
      if (savedModel?.graph?.layout !== layout) {
        savedModel?.graph?.layout && setLayout(savedModel?.graph?.layout);
      }
    }
  };
